...
CONNECTOR_NAME      ?= cm-saas-letsencrypt-ca-connector
LD_FLAGS            ?= "-w -s"
TAG                 ?= latest
...
help:
	...
	@echo "make format                cleanup import lines and format the code"
...
build: generate
	mkdir -p output/bin
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o output/bin/${CONNECTOR_NAME} ./cmd/$(CONNECTOR_NAME)/${CONNECTOR_MAIN_GO}

.PHONY: build-local
build-local: generate
	go install ./cmd/$(CONNECTOR_NAME)/${CONNECTOR_MAIN_GO}
